...
@WebServlet(
		...
		initParams = {  // initParams : 초기값을 설정할 수 있음
		...
		}) // @WebServlet을 사용할거면 web.xml에는 명시하면 안됨 >> Tomcat이 죽어버림
public class NowServlet extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		// 초기화 함수 (자동 호출되는 함수)
		// 호출 시점 : NowServlet 클래스에 대한 [최초 요청시 한번 실행] >> 진짜 딱! 한번만 실행됨 >> initParams를 통해 초기값을 생성!!
		// 재실행 : 서버의 재시작 , 개발자의 코드 수정
		
		// it.co.kr 서버 오픈 > WAS (서블릿 : NowServlet.java)
		// 홍길동 첫 접속자 -> it.co.kr/Now.do 서버 요청 
		// NowServlet 컴파일 >> class >> 실행 -> 생성자 호출 -> init 자동호출(이때만 실행!!!!!!) -> doGET or doPOST -> 자동호출
		...
		// DB연결 초기화 
		String drivername = config.getInitParameter("jdbcDriver"); // initParams를 가져옴
