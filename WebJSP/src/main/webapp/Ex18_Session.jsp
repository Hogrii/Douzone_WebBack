...
<body>
	...
	ex) Client  로그 아웃 버튼을 클릭 >> 서버는 어떤한 작업을 수행 할까요 >> 로그아웃 판단 >> key 가지고 와서
  	...
  	<hr>
  	<%
  		time.setTime(session.getCreationTime()); // session이 처음 만들어진 시간
  	%>
  	[session이 생성된 시간] : <%= formatter.format(time) %>
  	<hr>
  	<%
  		time.setTime(session.getLastAccessedTime()); // session의 마지막 접속 시간
  	%>
  	[session의 마지막 접속 시간] : <%= formatter.format(time) %>
</body>
</html>